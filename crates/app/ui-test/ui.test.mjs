...
console.log("\nupdates");
{
  ...
  upd.error = "Set the release repository in Settings → Updates first";
  await window.eval("checkForUpdates()");
  await tick(80);
  ok(doc.querySelector('.sheet [data-num="updates.repo"]'), "no repository set opens the Updates page");
  upd.error = null;
  closeSheetIfOpen();
}
